...
이제 이 두 클래스를 연결야 한다. Unity 프로젝트에 가서 새로운 C# 파일을 만들고 이름을 `MyGameLifetimeScope.cs` 로 설정해 둔다. 사실 다른 이름이어도 별 상관없지만, `~ LifetimeScope.cs` 로 끝나는 파일을 만들면 자동으로 코드 템플릿이 적용되기 때문에 편리하기 떄문이다. 그리고, `Configure` 메서드에 두 클래스를 등록해 둔다. 이를 통해 두 클래스를 등록해둔다.
